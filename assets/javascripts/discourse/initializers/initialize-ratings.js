...
export default {
  ...
  initialize(container) {
    ...
    withPluginApi("1.14.0", (api) => {
      ...
      // Use new API for post properties
      api.addTrackedPostProperties("ratings");

      // Glimmer component version for post stream
      api.renderAfterWrapperOutlet(
        "post-meta-data-poster-name",
        class extends Component {
          static shouldRender(args) {
            return (
              args.post?.topic?.show_ratings &&
              args.post?.ratings?.length > 0
            );
          }
      ...
      // Add CSS class to posts with ratings using value transformer
      ...
      // Legacy widget support (will be removed in future versions)
        ...
        api.decorateWidget("poster-name:after", function (helper) {
          ...
          if (post && post.topic && post.topic.show_ratings && post.ratings) {
            return new RenderGlimmer(
              this,
              "div.post-ratings",
              <template><RatingList @ratings={{@data.ratings}} /></template>,
              null,
